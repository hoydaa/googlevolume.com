...
$engine = new FlickrHitFetcher();

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/flickr.very.much.results.html'));
$t->is($result, 691594, 'Result count has to be 691594 for file flickr.very.much.results.html');

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/flickr.nothing.html'));
$t->is($result, 0, 'Result count has to be 0 for flickr.nothing.html');

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/flickr.armut.html'));
