...
$hit = $hitFetcher->fetch(file_get_contents(dirname(__FILE__).'/google.umut.utkan.html'));
$t->is($hit, 789, 'Result count has to be 789 for file google.umut.utkan.html');

$hit = $hitFetcher->fetch(file_get_contents(dirname(__FILE__).'/google.less.than.10.html'));
$t->is($hit, 1, 'Result count has to be 1 for google.less.than.10.html');

$hit = $hitFetcher->fetch(file_get_contents(dirname(__FILE__).'/google.nothing.html'));
$t->is($hit, 0, 'Result count has to be 0 for google.nothing.html');

$hit = $hitFetcher->fetch(file_get_contents(dirname(__FILE__).'/google.very.much.results.html'));
