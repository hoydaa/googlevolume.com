...
$engine = new YoutubeHitFetcher();

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/youtube.very.much.results.html'));
$t->is($result, 25200, 'Result count has to be 25200 for file youtube.very.much.results.html');

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/youtube.nothing.html'));
$t->is($result, 0, 'Result count has to be 0 for youtube.nothing.html');

$result = $engine->extractHit(file_get_contents(dirname(__FILE__).'/youtube.armut.html'));
